...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if(mappages(new, i, PGSIZE, pa, flags) != 0) {
      // If mapping fails, restore parent's PTE if we modified it
      if(flags & PTE_COW) {
...
// Handle COW page fault: allocate a new page, copy the old page, and update PTE
int
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
    ...
    // Handle COW page: if PTE_COW is set, allocate a new page first
    if(*pte & PTE_COW) {
